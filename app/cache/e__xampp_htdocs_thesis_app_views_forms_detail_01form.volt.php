...
<div class=WordSection1>
    ...
    <h1 style='margin-top:12.0pt;margin-right:0in;margin-bottom:.25in;margin-left:0in'><span style='font-size:14.0pt'>ĐƠN XIN HOÃN THI LẦN
            <?php if (isset($form_infor['number'])) { ?>
                <?php echo $this->tag->numericField(array('number', 'value' => $form_infor['number'])); ?>
            <?php } else { ?>
                <?php echo $this->tag->numericField(array('number')); ?>
            <?php } ?>
        </span></h1>
    ...
    <h2><span style='line-height:150%'>Xin hoãn thi học phần:
            <?php if (isset($form_infor['term'])) { ?>
                <?php echo $this->tag->textField(array('term', 'value' => $form_infor['term'])); ?>
            <?php } else { ?>
                <?php echo $this->tag->textField(array('term')); ?>
            <?php } ?>
        </span></h2>

    <h2><span style='line-height:150%'>Giảng viên giảng dạy:
            <?php if (isset($form_infor['teacher'])) { ?>
                <?php echo $this->tag->textField(array('teacher', 'value' => $form_infor['teacher'])); ?>
            <?php } else { ?>
                <?php echo $this->tag->textField(array('teacher')); ?>
            <?php } ?>
        </span></h2>

    <p class=MsoNormal style='line-height:150%'><span style='font-size:14.0pt;line-height:150%'>Thi ngày:
            <?php if (isset($form_infor['date'])) { ?>
                <?php echo $this->tag->dateField(array('date', 'value' => $form_infor['date'])); ?>
            <?php } else { ?>
    ...
    <p class=MsoHeader style='line-height:150%'><span style='line-height:150%'>Lý do:
            <?php if (isset($form_infor['reason'])) { ?>
                <?php echo $this->tag->textField(array('reason', 'value' => $form_infor['reason'])); ?>
            <?php } else { ?>
                <?php echo $this->tag->textField(array('reason', 'value' => $form_infor['reason'])); ?>
            <?php } ?>
        </span></p>
    <?php echo $this->tag->submitButton(array('Send', 'name' => 'send')); ?>
